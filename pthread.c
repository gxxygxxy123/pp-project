...
void *calculate(void *thread_arr){
    thread_s *p = (thread_s*)thread_arr;
    for(int i = p->thread_id;i < p->cluster->num_clusters;i+=p->thread_num){
        cluster_node_t *node = &(p->cluster->nodes[p->nodes_available[i]]);
        neighbour_t *neighbour = node->neighbours;
        while (neighbour) {
                if (p->cluster->nodes[neighbour->target].is_root) {
                        if (p->first == -1 ||
                            neighbour->distance < p->best_distance) {
                                p->first = p->nodes_available[i];
                                p->second = neighbour->target;
                                p->best_distance = neighbour->distance;
                        }
                }
                neighbour = neighbour->next;
        }
...
int find_clusters_to_merge(cluster_t *cluster, int *first, int *second)
{
        ...
        *first = -1;
        int thread_num = thread_you_want;
        pthread_t *thread = (pthread_t*)malloc(thread_num * sizeof(pthread_t));
        thread_s *thread_arr = (thread_s*)malloc(thread_num * sizeof(thread_s));
        
        // build available nodes (neighbours)
        int *nodes_available = (int*)malloc(cluster->num_clusters * sizeof(int));
        ...
        for(int i = 0;i < thread_num;i++){
            thread_arr[i].cluster = cluster;
            thread_arr[i].nodes_available = nodes_available;
            thread_arr[i].thread_id = i;
            thread_arr[i].thread_num = thread_num;
            thread_arr[i].first = -1;
            thread_arr[i].second = -1;
            thread_arr[i].best_distance = FLT_MAX;
            pthread_create(&thread[i], NULL, calculate, &thread_arr[i]);
        }
        for(int i = 0;i < thread_num;i++){
        ...
        for(int i = 0;i < thread_num;i++){
            if(*first == -1 || thread_arr[i].best_distance < best_distance){
                *first = thread_arr[i].first;
                *second = thread_arr[i].second;
                best_distance = thread_arr[i].best_distance;
            }
        }
        free(thread);
        free(thread_arr);
        free(nodes_available);
        return *first;
}
...
int main(int argc, char **argv)
{
        if (argc != 5) {
                fprintf(stderr, "Usage: %s <input file> <num clusters> "
                        "<linkage type> <thread number>\n", argv[0]);
                exit(1);
        } else {
                thread_you_want = atoi(argv[4]);
                item_t *items = NULL;
