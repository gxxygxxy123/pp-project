...
//gettimeofday(&end, 0); 
//sec=end.tv_sec-start.tv_sec; usec=end.tv_usec-start.tv_usec;  pass_time=sec+(usec/1000000.0);
//add_time += pass_time;
        return neighbour;
}
        ...
        for(int i = 0;i < cluster->num_clusters;i++){
			...
			while (neighbour) {
	                if (cluster->nodes[neighbour->target].is_root) {
	                        if (ans_first == -1 ||
	                            neighbour->distance < ans_distance) {
	                                ans_first = nodes_available[i] ;
	                                ans_second = neighbour->target;
	                                ans_distance = neighbour->distance;
	                        }
	                }
	                neighbour = neighbour->next;
	        }
	        #pragma omp critical
			{
				if(ans_distance < best_distance){
					*first = ans_first;
					*second = ans_second;
					best_distance = ans_distance;
				}
			}
		}
