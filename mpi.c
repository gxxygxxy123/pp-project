...
struct neighbour_s {
        ...
        neighbour_t *next; /* linked list entries */
};
...
neighbour_t *add_neighbour(cluster_t *cluster, int index, int target)
{
        ...
        if (neighbour) {
                ...
                if (node->neighbours){
                    neighbour->next = node->neighbours->next;
                    node->neighbours->next = neighbour;
                }
...
int find_clusters_to_merge(cluster_t *cluster, int *first, int *second)
{
        ...
        */
        int *nodes_available = malloc(cluster->num_clusters * sizeof(int));
        for(int i = cluster->num_nodes-1, node_idx = 0;i >= 0;i--){
            cluster_node_t *node = &(cluster->nodes[i]);
            if(!(node->type == NOT_USED || !node->is_root)){
                nodes_available[node_idx++] = i;
            }
            if(node_idx >= cluster->num_clusters){
                break;
            }
        }
        ...
        MPI_Status status;
        int left = rank;
        int right = cluster->num_clusters;
        int partial_first = -1, partial_second = -1;
        float partial_best_distance = FLT_MAX;
        for(int i = left ; i < right ; i += size){
            cluster_node_t *node = &(cluster->nodes[nodes_available[i]]);
            neighbour_t *neighbour = node->neighbours;
            while (neighbour) {
                if (cluster->nodes[neighbour->target].is_root) {
                        if (partial_first == -1 ||
                            neighbour->distance < partial_best_distance) {
                                partial_first = nodes_available[i];
                                partial_second = neighbour->target;
                                partial_best_distance = neighbour->distance;
                        }
                }
                neighbour = neighbour->next;
            }
        ...
        if(rank == 0){
            *first = partial_first;
            *second = partial_second;
            best_distance = partial_best_distance;
            for(int i = 1; i < size; i++){
                MPI_Recv(&partial_first, 1, MPI_INT, i, 0, MPI_COMM_WORLD, &status);
                MPI_Recv(&partial_second, 1, MPI_INT, i, 1, MPI_COMM_WORLD, &status);
                MPI_Recv(&partial_best_distance, 1, MPI_FLOAT, i, 2, MPI_COMM_WORLD, &status);

                if(*first == -1 || partial_best_distance < best_distance){
                    *first = partial_first;
                    *second = partial_second;
                    best_distance = partial_best_distance;
                }
            ...
            for(int i = 1;i < size; i++){
                int tmp_first = *first, tmp_second = *second;
                //printf("myrank: %d   first: %d   second: %d  num_c:%d \n", rank, *first, *second, cluster->num_clusters);
                MPI_Send(&tmp_first, 1, MPI_INT, i, 87, MPI_COMM_WORLD);
                MPI_Send(&tmp_second, 1, MPI_INT, i, 88, MPI_COMM_WORLD);
                //MPI_Send(&best_distance, 1, MPI_FLOAT, i, 2, MPI_COMM_WORLD);
            }
        }
        else{
            MPI_Send(&partial_first, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
            MPI_Send(&partial_second, 1, MPI_INT, 0, 1, MPI_COMM_WORLD);
            MPI_Send(&partial_best_distance, 1, MPI_FLOAT, 0, 2, MPI_COMM_WORLD);
            int tmp_first, tmp_second;
            MPI_Recv(&tmp_first, 1, MPI_INT, 0, 87, MPI_COMM_WORLD, &status);
            
            MPI_Recv(&tmp_second, 1, MPI_INT, 0, 88, MPI_COMM_WORLD, &status);

            *first = tmp_first;
            *second = tmp_second;
            //printf("myrank: %d   first: %d   second: %d  \n", rank,  *first,  *second);
            //MPI_Recv(&best_distance, 1, MPI_FLOAT, i, 2, MPI_COMM_WORLD, &status);
        }
